    ...
    <header x-data="{ isOpen: false, isCollapsed: false }"
        ...
        <div class="px-6 py-2 flex items-center justify-between transition-all duration-500"
            ...
            <div class="hidden md:flex md:items-center md:gap-5 transition-all duration-300"
                :class="isCollapsed ? 'opacity-0 scale-90 overflow-hidden' : 'opacity-100 scale-100'">
                <a href="#" class="rounded-lg px-2 py-1 text-sm font-medium text-gray-900 hover:bg-gray-100">
                    Home
                </a>
                <a class="rounded-lg px-2 py-1 text-sm font-medium text-gray-900 hover:bg-gray-100" href="#">
                    Cek Data
                </a>
            </div>

            <!-- Tombol Login & Registrasi (Desktop) -->
        ...
        <button @click="isCollapsed = !isCollapsed; isOpen = false"
            ...
            <template x-if="!isCollapsed">
                <i class="fa-solid fa-angles-up p-1"></i>
            </template>
            <template x-if="isCollapsed">
                <i class="fa-solid fa-angles-down p-1"></i>
            </template>
        </button>

        <!-- Menu Dropdown (Mobile) -->
        <div x-show="isOpen" x-transition class="md:hidden bg-white shadow-lg py-4 mt-2 p-4 rounded-lg overflow-hidden">
            <a class="block px-4 py-2 text-gray-900 hover:bg-gray-100 hover:rounded-md w-full text-center" href="#">Home</a>
            <a class="block px-4 py-2 text-gray-900 hover:bg-gray-100 hover:rounded-md w-full text-center" href="#">Cek Data</a>
            <a class="block px-4 py-2 text-blue-600 font-semibold hover:bg-gray-100 hover:rounded-md w-full text-center"
                href="/login">Registrasi</a>
            <a class="block px-4 py-2 bg-blue-600 text-white font-semibold rounded-md w-full text-center hover:bg-blue-500"
                href="/login">Masuk</a>
        </div>
    </header>

    <!-- Content -->
    <main class="flex-grow min-h-screen relative">
        @yield('content')
    </main>

    <!-- Footer -->
    <footer class="bg-white/80 py-4 text-center w-full">
        <a href="https://smkmuhpakem.sch.id/" target="_blank" rel="noopener noreferrer"
            class="flex flex-col sm:flex-row items-center justify-center mb-3 text-lg sm:text-xl font-semibold text-gray-900">
            <img src="{{ asset('storage/img/logo.png') }}" class="h-10 sm:h-8 mr-0 sm:mr-3" alt="smkmupa" />
            <span class="text-sm sm:text-base">SMK Muhammadiyah Pakem</span>
        </a>

        <span class="text-xs sm:text-sm text-gray-600">
            © 2024 SMK Muhammadiyah Pakem. All rights reserved.
        </span>
